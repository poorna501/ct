...
var ButtonArr = [];
var flag = firstWhile = testingFlag = true, ch = "y", name; 
...
PolynomialLL.prototype.storeArrayValues = function() {
	if (flag) {
...
PolynomialLL.prototype.create = function(head) {
	...
	if (flag) {
		...
		this.cmd("Sethighlight", this.head2RectID, "#ff4d4d");
		this.cmd("Step");
		this.cmd("Step");
...
PolynomialLL.prototype.yesOrNoQus = function() {
	this.commands = new Array();
...
PolynomialLL.prototype.addTerm = function(head, t) {
	...
	console.log("flag in addTerm " + flag);
	console.log("buttonName in addTerm " + buttonName);
	let index = 0;
...
PolynomialLL.prototype.print = function(head) {
    ...
    console.log("flag in print " + flag);
...
PolynomialLL.prototype.createAnEmptyNode = function() {
	...
	randomAdd = getRandomInt(1000, 5000);
	address[nodeCount] = addTermAdd[nodeCount] = randomAdd;
...
PolynomialLL.prototype.addition = function(head1, head2, operation) {
	...
	if (t1 == null && t2 == null) {
		...
		if (t1 == null) {
			indexM1 = index1;
			if(address1.length != 0 && indexM1 != address1.length) {
				testingFlag = false;
			...
			while(t1 != null) {
				...
				console.log("coount in second " + coount);
				this.cmd("Step");
				t3 = {};
				coount = 2;
				firstWhile = false;
		...
		if (t2 == null) {
			...
			while(t2 != null) {
				...
				console.log("coount in third " + coount);
				this.cmd("Step");
				t3 = {};
				coount = 3;
				firstWhile = false;
			...
			this.cmd("Step");
			this.introNextStep("#rtnSum", "right", "hide");
			this.cmd("Step");
		}
	}
...
PolynomialLL.prototype.T1ExpGraterAndT1NotNull = function(index1) {
	...
	this.cmd("Step");
	firstAdd = (sum == null) ? "NULL" : address[0];
...
PolynomialLL.prototype.multiplicationAnimation = function() {
	...
	$('#headsDes').removeAttr("style").empty().append('\n<div class="opacity00" id="multiplicationInMain">'
			+ '\t<span id="addCll1">head3 = <span id="mthdNme">mul</span>(<span id="head1Name" class="position">head1</span>,'
			+ ' <span id="head2Name" class="position">head2</span> );</span></span>\n'
			+ '\t<span id="printCall2">print(<span id="head3Name" class="position">head3</span>);</span></span>\n</div>');
	this.cmd("Step");
	this.introNextStep("#multiplicationInMain", "bottom", "hide");
	
	this.createTempararyNodes("pro");
	this.cmd("Step");
	this.introNextStep("#mulLogic", "right", "hide");
	this.cmd("Step");
	this.assignT1Value();
	this.cmd("Step");
	this.idChange();
