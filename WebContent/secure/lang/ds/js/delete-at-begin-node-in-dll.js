			...
			case 'animationDiv' :
				...
				switch(animateStep) {
					...
					case 'thirdExample' :
						...
						$('.introjs-tooltip').removeClass('hide');
						text = "<ul><li>Let us assume <y>doubly linked list</y> can contains more than <y>one node</y>."
			...
			case 'algorithmStepsDiv' :
				...
				switch(animateStep) {
					...
					case 'step2' :
						...
						appendSteps(2);
							var text = "<ul><li>If it is <brown>Not Empty</brown> then, define one tempary node <g>temp</g>"
								...
								+ "<g>data</g> field of <g>temp</g> and delete <g>temp</g> node and terminate the function."
					...
					case "step3":
						...
						appendSteps(3);
						var text = "<ul><li>If it is <brown>not NULL</brown> then assign <g>next</g> field of <g>first</g> to " 
									+ " <bwbg>first</bwbg> node and also assign <g>prev</g> field of <bwbg>first</bwbg> to <g>NULL</g> " 
									+ " then print <g>data</g> field of <g>temp</g> and delete <g>temp</g> node and terminate the function."
									+ "<span id='btn'></span></li></ul>";
						$('#stepDes3').html(text);
					...
					case "second" :
						$('#algorithmStepsDiv ul:last').effect( "highlight",{color: 'yellow'}, 600, function() {
							$('#ifFirstEqNull').after('<span id="elseBlock" class="opacity00"><span id="elseCon"> else {</span>\n'
									+ '\t\t\tnode temp = first;\n'
									+ '\t\t\tif (<span id="tempNextNull">temp -> next == NULL</span>) {\n'
									+ '\t\t\t\tfirst = NULL;\n'
...
function deleteAtBegin() {
	$('#parentPre').append('<span id="funName">node <g>deleteAtBegin()</g> { '
			+ '\n\t<span id="ifFirstEqNull" class="opacity00"> if (<span id="firstNullCon"><brown>first == NULL</brown></span>) {'
...
function firstNextToFirstText() {
	...
	if ($(".nodes").length == 1) {
		var text = "<li>Now check the <y>next</y> field of <y>temp</y> is <y>equal</y> to <y>NULL</y> or not.</li> "
					+ "<li>Here, the <y>next</y> field of <y>temp</y> is <y>NULL</y> so assign <y>NULL</y> to <y>first</y> node.</li>";
	} else {
		var text = "<li>Now check the <y>next</y> field of <y>temp</y> is <y>equal</y> to <y>NULL</y> or not.</li> "
					+ "<li>Here, the <y>next</y> field of <y>temp</y> is not equal to <y>NULL</y> so assign the "
					+ "<y>next</y> field of <y>first</y> to <y>first</y> node. and also assign <y>prev</y> field of <y>first</y> "
					+ "to <y>NULL</y>.</li>";	
	}
...
function applyBesidestep() {
	$('.user-btn, #btn').remove();
	text = '<li>Check if <y>first</y> is <y>equal</y> to <y>NULL</y> or not. Here the <y>first</y> node contains <y>'
			+ $('#firstVal').text() +'</y> is not a <y>NULL</y> so condition is evaluates to <y>false</y>.</li>';
	typing('#appendDiv', text, function() {
...
function applySecondstep() {
	...
	$('.user-btn, #btn').remove();
	var text = "<li>The above condition is <y>false</y> so declare one temporary node <y>temp</y> and initialize with <y>first</y> value"
				+ " (i.e. <y>"+ $('#firstVal').text()  +"</y>).</li>"
	typing('#appendDiv11', text, function() {
		
		
		//$('#tempNextNull').addClass('blinkingRed');
